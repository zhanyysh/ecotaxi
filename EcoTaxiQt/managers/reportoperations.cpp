...
QVariantList ReportOperations::getCarsReport(QDate fromDate, QDate toDate)
{
    ...
    QString query =
        ...
        "),\n"
        "grouped_charges AS (\n"
        "    SELECT carId, SUM(COALESCE(kwh * kwh_multiplier, 0)) AS totalKwhCost\n"
        "    FROM charges\n"
        "    WHERE date BETWEEN '" +
        fromDate.toString("yyyy-MM-dd") + "' AND '" + toDate.toString("yyyy-MM-dd") +
        "'\n"
...
QVariantList ReportOperations::getInvestorsReport(QDate fromDate, QDate toDate)
{
    ...
    QString query =
        ...
        "    SELECT\n"
        "        cars.id AS carId,\n"
        "        SUM(COALESCE(charges.kwh, 0)) AS totalKwh\n"
        "    FROM charges\n"
        ...
        "    FLOOR(SUM(\n"
        "        (COALESCE(event_stats.income, 0) * 0.95 - COALESCE(charge_stats.totalKwh, 0) * 10 + COALESCE(event_stats.outcome, 0))\n"
        "    )) AS totalProfit,\n"
        "    FLOOR(SUM(\n"
        "        CASE \n"
        "            WHEN (COALESCE(event_stats.income, 0) * 0.95 - COALESCE(charge_stats.totalKwh, 0) * 10 + COALESCE(event_stats.outcome, 0)) >= 0 THEN\n"
        "                (COALESCE(event_stats.income, 0) * 0.95 - COALESCE(charge_stats.totalKwh, 0) * 10 + COALESCE(event_stats.outcome, 0)) \n"
        ...
        "            ELSE (COALESCE(event_stats.income, 0) * 0.95 - COALESCE(charge_stats.totalKwh, 0) * 10 + COALESCE(event_stats.outcome, 0))\n"
        "        END\n"
        "    )) AS investorsIncome\n"
        "FROM investors\n"
