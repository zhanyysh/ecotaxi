...
QVariantList ReportOperations::getInvestorsReport(QDate fromDate, QDate toDate)
{
    ...
    QString query =
        ...
        "    FLOOR(SUM(\n"
        "        (COALESCE(event_stats.income, 0) * 0.95 - COALESCE(event_stats.totalDebt, 0) - COALESCE(charge_stats.totalKwh, 0) - ABS(COALESCE(event_stats.outcome, 0)))\n"
        "    )) AS totalProfit,\n"
        "    FLOOR(SUM(\n"
        "        CASE \n"
        "            WHEN (COALESCE(event_stats.income, 0) * 0.95 - COALESCE(event_stats.totalDebt, 0) - COALESCE(charge_stats.totalKwh, 0) - ABS(COALESCE(event_stats.outcome, 0))) >= 0 THEN\n"
        "                (COALESCE(event_stats.income, 0) * 0.95 - COALESCE(event_stats.totalDebt, 0) - COALESCE(charge_stats.totalKwh, 0) - ABS(COALESCE(event_stats.outcome, 0))) \n"
        ...
        "            ELSE (COALESCE(event_stats.income, 0) * 0.95 - COALESCE(event_stats.totalDebt, 0) - COALESCE(charge_stats.totalKwh, 0) - ABS(COALESCE(event_stats.outcome, 0)))\n"
        "        END\n"
        "    )) AS investorsIncome\n"
        "FROM investors\n"
...
QVariantList ReportOperations::getInvestorReport(int investorId, QDate fromDate, QDate toDate)
{
    ...
    QString query =
        ...
        "    percentage,\n"
        "    CASE \n"
        "        WHEN (FLOOR(income * 0.95 - kwh_with_multiplier + outcome - totalDebt) < 0) THEN \n"
        "            0 \n"
        "        ELSE \n"
        "            FLOOR((income * 0.95 - kwh_with_multiplier + outcome - totalDebt) * COALESCE(percentage, 0) / 100) \n"
        "    END AS ourProfit,\n"
        "    CASE \n"
        "        WHEN (FLOOR(income * 0.95 - kwh_with_multiplier + outcome - totalDebt) < 0) THEN \n"
        "            FLOOR(income * 0.95 - kwh_with_multiplier + outcome - totalDebt) \n"
        "        ELSE \n"
        "            FLOOR((income * 0.95 - kwh_with_multiplier + outcome - totalDebt) * COALESCE(100 - percentage, 0) / 100) \n"
        "    END AS investorsProfit,\n"
        "    totalDebt\n"
        "FROM car_stats;";
...
QVariantList ReportOperations::getAllInvestorReport(int investorId, QDate fromDate, QDate toDate)
{
    ...
    QString query =
        ...
        "        FLOOR(CASE \n"
        "            WHEN (COALESCE(event_stats.income, 0) * 0.95 + COALESCE(event_stats.outcome, 0) - COALESCE(charge_stats.kwh, 0) - COALESCE(event_stats.totalDebt, 0)) < 0 THEN 0 \n"
        "            ELSE (COALESCE(event_stats.income, 0) * 0.95 + COALESCE(event_stats.outcome, 0) - COALESCE(charge_stats.kwh, 0) - COALESCE(event_stats.totalDebt, 0)) * (COALESCE(cars.percentage, 0) / 100) \n"
        "        END) AS ourProfit,\n"
        "        FLOOR(CASE \n"
        "            WHEN (COALESCE(event_stats.income, 0) * 0.95 + COALESCE(event_stats.outcome, 0) - COALESCE(charge_stats.kwh, 0) - COALESCE(event_stats.totalDebt, 0)) < 0 THEN \n"
        "                (COALESCE(event_stats.income, 0) * 0.95 + COALESCE(event_stats.outcome, 0) - COALESCE(charge_stats.kwh, 0) - COALESCE(event_stats.totalDebt, 0)) \n"
        "            ELSE \n"
        "                (COALESCE(event_stats.income, 0) * 0.95 + COALESCE(event_stats.outcome, 0) - COALESCE(charge_stats.kwh, 0) - COALESCE(event_stats.totalDebt, 0)) * ((100 - COALESCE(cars.percentage, 0)) / 100) \n"
        "        END) AS investorsProfit,\n"
        "        COALESCE(event_stats.totalDebt, 0) AS totalDebt\n"
        "    FROM investors\n"
        "    LEFT JOIN cars ON cars.investorId = investors.id\n"
        ...
        "    FLOOR(SUM(outcome)) AS totalOutcome,\n"
        "    FLOOR(SUM(income) * 0.95 - SUM(totalDebt) - SUM(kwh) - ABS(SUM(outcome))) AS totalProfit,\n"
